...
namespace Unity.Services.Ccd.Management.Http
{
    ...
    /// </summary>
    [Preserve]
    public class JsonObjectCollectionConverter : JsonConverter
    {
        ///<inheritdoc cref="JsonConverter"/>
        /// <summary>Convert a JsonObject to JToken.</summary>
        public override void WriteJson(JsonWriter writer, object value, JsonSerializer serializer)
        {
            ...
            if (type == typeof(Dictionary<string, JsonObject>))
            {
                jobjCollection = (Dictionary<string, JsonObject>) value;
            }
            else if (type == typeof(List<JsonObject>))
            {
                jobjCollection = (List<JsonObject>) value;
            }
            else if (type == typeof(List<List<JsonObject>>))
            {
                jobjCollection = (List<List<JsonObject>>) value;
            }
