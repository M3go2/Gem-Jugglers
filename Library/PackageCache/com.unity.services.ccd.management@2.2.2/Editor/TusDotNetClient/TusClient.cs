...
namespace Unity.Services.Ccd.Management
{
    ...
    public class TusClient
    {
                ...
                async reportProgress =>
                {
                    try
                    {
                        ...
                        while (offset < fileStream.Length)
                        {
                            ...
                            request.AddHeader(TusHeaderNames.UploadLength, bytesRead.ToString());
        ...
        internal static HttpClientResponse MapTusHttpResponsesToHttpResponse(List<TusHttpResponse> tusHttpResponses)
        {
            ...
            {
                return new HttpClientResponse(null, (int)HttpStatusCode.NoContent, false, false, null, null);
            }

            //Check for errors
            var finalResponse = tusHttpResponses.Last();
            foreach (var tusResponse in tusHttpResponses)
            {
                if (tusResponse.StatusCode >= HttpStatusCode.BadRequest)
                {
                    finalResponse = tusResponse;
                    break;
                }
            }
